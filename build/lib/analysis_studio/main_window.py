    ...
    def run(self) -> None:
        try:
            if self.backend == "local":
                ...
                executor = LSFExecutor(self.log.emit)
            else:
...
class BackendSettingsDialog(QDialog):
    def __init__(self, options: dict[str, object], parent=None) -> None:
        ...
        self.lsf_max_active_jobs = QSpinBox()
        self.lsf_max_active_jobs.setRange(0, 1_000_000)
        self.lsf_max_active_jobs.setValue(int(options.get("lsf_max_active_jobs", 500)))
        self.lsf_max_active_jobs.setSpecialValueText("Unlimited")
        form.addRow("LSF maximum active jobs", self.lsf_max_active_jobs)
    ...
    def values(self) -> dict[str, object]:
        return {
            ...
            "lsf_max_active_jobs": self.lsf_max_active_jobs.value(),
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.properties.set_backend(self.project.backend)
    ...
    def eventFilter(self, watched, event):  # noqa: N802
        # Guarantee project-level Ctrl+Y even when a child widget has a local
        # redo binding. The QAction remains responsible for menu activation and
        # platform-native redo sequences such as Ctrl+Shift+Z.
        if (
    ...
    def _create_actions(self) -> None:
        ...
        # QKeySequence.StandardKey.Redo may already resolve to Ctrl+Y on some
        # platforms. Registering Ctrl+Y a second time creates an ambiguous Qt
        # shortcut: the menu still works, but the keyboard event is discarded.
        redo_sequences: list[QKeySequence] = []
        ...
        for sequence in [
            *QKeySequence.keyBindings(QKeySequence.StandardKey.Redo),
            ...
            if portable and portable not in seen_sequences:
                seen_sequences.add(portable)
                redo_sequences.append(sequence)
        self.redo_action.setShortcuts(redo_sequences)
    ...
    def _backend_changed(self) -> None:
        ...
        if backend and backend != self.project.backend:
            ...
            self.properties.set_backend(backend)
            self._project_mutated()
