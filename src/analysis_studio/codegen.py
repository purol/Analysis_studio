...
from .analysis_modules import MODULE_SPECS, support_files
from .module_codegen import module_to_cpp
...
def loader_node_to_cpp(graph: Graph, node: WorkflowNode) -> list[str]:
    ...
    if node.type == "loader_decl":
        ...
        return [f"Loader {variable}({cpp_string(p['branch'])});"]
    if node.type == "raw_cpp":
        return str(p["code"]).splitlines()

    loader = _loader_variable(graph, node)
    if node.type in MODULE_SPECS:
        return module_to_cpp(loader, node)
...
def generate_loader_cpp(graph: Graph) -> str:
    ...
    for node in ordered:
        ...
        if node.type == "event_weight":
            weight = node.properties["weight"]
            lines = module_to_cpp(_loader_variable(graph, node), node, register_weight=weight not in registered_weights)
            if node.properties["weight_object"]:
                registered_weights.add(weight)
        else:
            lines = loader_node_to_cpp(graph, node)
        statements.extend(f"    {line}" if line else "" for line in lines)
        if node.type == "loader_end":
            for task in ordered:
                if task.type in MODULE_SPECS and loader_declaration_for_node(graph, task).id == declaration.id:
                    statements.extend(f"    {line}" for line in module_to_cpp(_loader_variable(graph, task), task, after=True))
        statements.append("")

    body = "\n".join(statements).rstrip()
    support = "\n".join(f"#include {cpp_string(path)}" for path in support_files(graph, "headers"))
    declarations = "\n".join(str(node.properties.get("declarations", "")) for node in ordered if node.type == "cpp_support")
