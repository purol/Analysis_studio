...
def _loader_names(graph: Graph) -> dict[str, str]:
    declarations = [
        node 
        for node in graph.ordered_roots() 
...
def loader_node_to_cpp(graph: Graph, node: WorkflowNode, loader_names: dict[str, str]) -> list[str]:
    p = node.properties
    if node.type == "loader_decl":
        loader = loader_names[node.id]
        return [f"Loader {loader}({cpp_string(p['tree'])});"]
    if node.type == "raw_cpp":
        return str(p["code"]).splitlines()

    declaration = loader_declaration_for_node(graph, node)
    loader = loader_names[declaration.id]
...
def generate_loader_cpp(graph: Graph) -> str:
    ...
    for index, declaration in enumerate(declarations, start = 1):
        statements.append(f"    // Loader chain {index}: {declaration.title}")

        chain_nodes = []

        for node in topological_nodes:
            if node.id == declaration.id:
                chain_nodes.append(node)
                continue
            
            try:
                node_declaration = loader_declaration_for_node(graph, node)
            except ValueError:
                continue
            
            if node_declaration.id == declaration.id:
                chain_nodes.append(node)
        ...
        for node in chain_nodes:
            ...
            statements.extend(
                f"    {line}" if line else "" for line in loader_node_to_cpp(graph, node, loader_names)
            )
            statements.append("")

    body = "\n".join(statements).rstrip()
    return f"""// Generated by Analysis Studio. Review before production use.
#include <stdio.h>
#include <string>
#include <vector>
#include <map>
