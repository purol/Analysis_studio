...
def compile_project(
    ...
    for node in project.workflow.nodes:
        if node.type != "custom_command":
            continue
        key = (str(node.properties.get("code", "")), custom_command_output_name(node))
        ...
        output = binary_dir / custom_command_output_name(node)
        ...
        if mode == "custom":
            ...
            environment.update(
                {
                    ...
                    "AS_BELLE2_ANALYSIS": str(
                        root / str(project.build_options.get("belle2_analysis_dir", "Belle2_analysis"))
                    ),
                }
            )
            log("$ " + command)
        ...
        elif mode == "copy" or suffix not in {".cc", ".cpp", ".cxx", ".c++"}:
            ...
            _compile_cpp(
                ...
                output,
                additional,
                _lines(node.properties.get("compile_flags", "")),
                _lines(node.properties.get("link_flags", "")),
                require_framework()
                if bool(node.properties.get("use_analysis_framework", True))
                else None,
                log,
                use_analysis_framework=bool(
                    node.properties.get("use_analysis_framework", True)
                ),
            )
